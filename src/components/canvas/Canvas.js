...
const defaultWorkareaOption = {
    ...
    scaleX: 1,
    scaleY: 1,
    backgroundColor: 'rgba(22, 100, 224, 0)',
    hasBorders: false,
    hasControls: false,
    selectable: false,
    lockMovementX: true,
...
class Canvas extends Component {
    ...
    componentDidMount() {
        const { id } = this.state;
        const { editable, canvasOption, workareaOption, guidelineOption, pdf, pdfCanvasOption, zoomRatio } = this.props;
        ...
        this.canvas.setBackgroundColor(mergedCanvasOption.backgroundColor, this.canvas.requestRenderAll.bind(this.canvas));
        const mergedWorkareaOption = Object.assign({}, defaultWorkareaOption, workareaOption, {
            workareaWidth: pdfCanvasOption.width,
            workareaHeight: pdfCanvasOption.height,
            ...pdfCanvasOption,
        });
        // console.log('> ', mergedCanvasOption);
        this.workarea = new fabric.Image(null, mergedWorkareaOption);
        if (pdf && !this.pdfImage) {
            const backgroundImage = new Image();
            backgroundImage.src = pdf;
            this.pdfImage = new fabric.Image(backgroundImage, {
                width: pdfCanvasOption.width * zoomRatio,
                height: pdfCanvasOption.height * zoomRatio,
                backgroundImageStretch: true,
            });
            this.canvas.setBackgroundImage(this.pdfImage);
        }
    ...
    componentDidUpdate(prevProps) {
        if (JSON.stringify(this.props.canvasOption) !== JSON.stringify(prevProps.canvasOption)) {
            // console.log('> 1', this.props.canvasOption, prevProps.canvasOption);
            ...
            this.canvas.setBackgroundColor(this.props.canvasOption.backgroundColor, this.canvas.requestRenderAll.bind(this.canvas));
    ...
    objectHandlers = {
        groupByAssignmentProperties: objects => {
            if (!objects.length) return {};
            // field look up to be
            const propertiesGroupToBe = [
                'groupAssignment',
                'individualAssignment',
            ];
            return _.chain(propertiesGroupToBe).reduce((accum, property) => {
                const grouped = _.chain(objects)
                                .map(o => o[property]) // find out all of properties they have
                                .flatten()
                                .compact() // remove all of null members
                                .uniqBy(o => String(o)) // use distinct members
                                .value();
                return {
                    ...accum,
                    [property]: grouped,
                    isAssignment: !!grouped.length,
                };
            }, {}).value();
        },
    };
    ...
    dormerHandlers = {
        createDormer: obj => {
            ...
            // register event
            // document.addEventListener('keydown', this.eventHandlers.dormerEscKeyPress, false);
            // document.addEventListener('dbclick', this.eventHandlers.dormerDbClick, false);
        },
        getAreaInfo: (obj, code, hide) => {
            let {
                html,
            } = code;
            const {
            ...
            if (obj.shortcut) {
                html = _.replace(html, new RegExp('{{__viewMode__}}', 'g'), 'inline-block');
            } else {
                html = _.replace(html, new RegExp('{{__viewMode__}}', 'g'), 'none');
            }
            ...
            html = _.replace(html, new RegExp('{{__tlabel__}}', 'g'), i18n.t('common.shortcut'));
            html = _.replace(html, new RegExp('{{__editlabel__}}', 'g'), i18n.t('common.edit'));
            html = _.replace(html, new RegExp('{{__desc__}}', 'g'), obj.description);
            html = _.replace(html, new RegExp('{{__title__}}', 'g'), obj.title);
            return {
                code: {
                    html,
                    css,
    ...
    handlers = {
        ...
        addImage: (obj, centered = true, loaded = false) => {
            const { editable, zoomRatio } = this.props;
            ...
            const createImage = (img) => {
                ...
                if (obj.ignored) {
                    this.dormerHandlers.createDormer(createdObj);
                }
                if (!editable) {
                    createdObj.on('mousedown', this.eventHandlers.object.mousedown);
                }
        ...
        addElement: (obj, centered = true, loaded = false) => {
            ...
            if (obj.type === 'element') {
                const infoOptions = this.dormerHandlers.getAreaInfo(obj, code);
                rectOptions = {
                    ...rectOptions,
                    ...infoOptions,
                };
            ...
            const createdObj = new fabric.Rect(rectOptions);
            if (!editable) {
                createdObj.on('mousedown', this.eventHandlers.object.mousedown);
            }
            ...
            if (src || file || code) {
                if (obj.type === 'video') {
                    ...
                    this.elementHandlers.set(createdObj, src || rectOptions.code);
                }
            }
            if (editable && !loaded) {
                this.handlers.centerObject(createdObj, centered);
        ...
        showById: (id) => {
            ...
            if (findObject) {
                ...
                if (findObject.areaMode) {
                    const {
                        option:{
                            code,
                        },
                    } = this.props.descriptor.LINKAREA[0];
                    const info = this.dormerHandlers.getAreaInfo(findObject, code);
                    this.elementHandlers.set(findObject, info.code);
                }
                this.canvas.requestRenderAll();
            }
        },
        hideById: (id) => {
            ...
            if (findObject) {
                ...
                if (findObject.areaMode) {
                    ...
                    const info = this.dormerHandlers.getAreaInfo(findObject, code, true);
                    this.elementHandlers.set(findObject, info.code);
                }
                this.canvas.requestRenderAll();
            }
        ...
        set: (key, value, obj) => {
            const activeObject = obj || this.canvas.getActiveObject();
            if (!activeObject) {
                return false;
            }
        ...
        // scaleToResizes: (width, height, objs) => {
        //     _.forEach(objs, obj => {
        //         const o = {
        //             id: obj.id,
                ...
                json.forEach((obj) => {
                    ...
                    if (this.handlers.isElementType(obj.type)) {
                        ...
                        if (obj.type === 'element') {
                            ...
                            const info = this.dormerHandlers.getAreaInfo(obj, code);
                            obj.code = info.code;
                            obj.areaMode = true;
                        }
                    }
                    ...
                    this.canvas.requestRenderAll();
                    if (!this.props.canvasOption.selection) {
                        this.handlers.disableAll();
                    }
                });
                if (callback) {
        ...
        getSelectedObjects: () => this.canvas.getActiveObjects(),
        ...
        toGroup: () => {
            ...
            // activeObjects = _.filter(activeObjects, o => !o.singleton);
            // const group = new fabric.Group(activeObjects);
            // canvas.add(group);
            // canvas.setActiveObject(group);
            // canvas.requestRenderAll();
            // if (activeObjects.length) {
            //     // remove all of singleton objects
            //     activeObjects = _.filter(activeObjects, o => !o.singleton);
            //     _.forEach(activeObjects, o => {
            //         group.addWithUpdate(o);
            //         canvas.remove(o);
            //     });
            //     // selection = new fabric.Group(activeObjects, {
            //     //     canvas,
            //     // });
            //     canvas.setActiveObject(group);
            //     canvas.add(group);
            ...
            const assignmentGrouped = this.objectHandlers.groupByAssignmentProperties(canvas.getActiveObjects());
            const confirmed = true;
            if (assignmentGrouped.isAssignment) {
                confirmed = window.confirm(i18n.t('message.confirmAssignmentGroup'));
            }
            if (confirmed) {
        ...
        saveImage: (option = { name: 'Default', format: 'png', quality: 1 }) => {
            const dataUrl = this.canvas.toDataURL(option);
            console.log('>>>> dataUrl', option);
    ...
    elementHandlers = {
        viewArea: obj => {
            // console.log(obj);
            if (obj.shortcut) {
                this.props.onClickHyperLink(obj.shortcut);
            }
        },
        editArea: obj => {
            // console.log(obj);
            this.props.switchProperties('shortcut');
        },
        setById: (id, source) => {
            const findObject = this.handlers.findById(id);
            if (!findObject) {
        ...
        createElement: (obj, code) => {
            ...
            //     const editTarget = element.getElementsByClassName('edit_evt');
            //     const viewTarget = element.getElementsByClassName('view_evt');
            //     console.log(editTarget, viewTarget);
            // });
            
            // if (editTarget) {
            //     editTarget.onclick = () => {
            //         console.log(obj);
            //         this.elementHandlers.editArea(obj);
            //     }
            // }
            // if (viewTarget) {
            //     viewTarget.onclick = () => {
            //         this.elementHandlers.viewArea(obj);
            //     }
            // }
            ...
            ** Panel Button Events
            */
            const editTarget = element.getElementsByClassName('edit_evt')[0];
            const viewTarget = element.getElementsByClassName('view_evt')[0];
            if (editTarget) {
                editTarget.onclick = () => {
                    this.elementHandlers.editArea(obj);
                }
    ...
    zoomHandlers = {
        zoomToPoint: (point, zoom, ignoredCallback) => {
        ...
        zoomToDefault: ignoredCallback => {
            const center = this.canvas.getCenter();
            const point = {
                x: center.left,
                y: center.top,
            };
            this.zoomHandlers.zoomToPoint(point, 1, ignoredCallback);
        },
        zoomMinimize: () => {
    ...
    eventHandlers = {
        ...
        mousedown: (opt) => {
            // on canvas blank clicking, hide the side menu
        ...
        selection: (opt) => {
            ...
            // console.log(activeObjects.removeWithUpdate);
        ...
        resize: (currentWidth, currentHeight, nextWidth, nextHeight) => {
            ...
            // console.log(this.canvas.width, this.canvas.height, this.canvas);
        ...
        keydown: (e) => {
            // console.log(this.canvas.getPointer(e.e));
            if (this.canvas.wrapperEl !== document.activeElement) {
                return false;
            }
            const { keyEvent } = this;
        ...
        onmousedown: (e) => {
            const isRightClick = this.eventHandlers.isRightClick(e);
            if (!isRightClick) {
                setTimeout(() => {
